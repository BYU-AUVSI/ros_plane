                                    ...
                                    int subpath_index)
{
  ...
  {
    if (-waypoints_[idx_b].w[2] < params.landing_alt)
      return generate_landing_path(params, idx_a, idx_b, subpath_index);
...
path_manager_example::Path
path_manager_example::generate_landing_path(const struct params_s &params, int idx_a, int idx_b, int path_index)
{
  Path path;

  ROS_WARN("LANDING PATH GENERATING");
  Eigen::Vector3f waypoint_start(waypoints_[idx_a].w);
  ROS_WARN("waypoint_start (%f, %f, %f)", waypoint_start(0), waypoint_start(1), waypoint_start(2));
  Eigen::Vector3f waypoint_land(waypoints_[idx_b].w);
  ROS_WARN("waypoint_land (%f, %f, %f)", waypoint_land(0), waypoint_land(1), waypoint_land(2));
  Eigen::Vector3f landing_unit = (waypoint_land - waypoint_start).normalized();
  landing_unit(2) = tan(params.landing_angle_deg * M_PI / 180.0);
  landing_unit = landing_unit.normalized();
  Eigen::Vector3f waypoint_final_start =
      waypoint_land - params.landing_alt / tan(params.landing_angle_deg * M_PI / 180.) * landing_unit;
  ROS_WARN("waypoint_final_start (%f, %f, %f)", waypoint_final_start(0), waypoint_final_start(1),
           waypoint_final_start(2));
  Eigen::Vector3f approach_unit = (waypoint_final_start - waypoint_start).normalized();
  Eigen::Vector3f end_point, start_point, unit_direction;

  path.output.flag = true;
  path.output.q[0] = landing_unit(0);
  path.output.q[1] = landing_unit(1);
  path.num_paths = 2;

  if (path_index == 0) // approach
  {
    path.output.Va_d = params.landing_va;
    start_point = waypoint_start;
    end_point = waypoint_final_start;
    unit_direction = approach_unit;
  } else // final
  {
    path.output.Va_d = 0.;
    start_point = waypoint_final_start;
    end_point = waypoint_land;
    unit_direction = landing_unit;
  }

  path.end_plane.point = end_point;

  path.output.r[0] = start_point(0);
  path.output.r[1] = start_point(1);
  path.output.r[2] = start_point(2);

  path.output.line_end[0] = end_point(0);
  path.output.line_end[1] = end_point(1);
  path.output.line_end[2] = end_point(2);

  path.end_plane.normal = unit_direction;
}
