...
namespace rosplane
{
            ...
            nh_private_(ros::NodeHandle("~"))
    {
        ...
        {
            gps_init_ = true;
            if (!nh_private_.hasParam("ins_lat_ref"))
            {
                gps_init_ = false;
                ROS_ERROR("Missing parameter for Inertial Sense: ~ins_lat_ref");
            }
            if (!nh_private_.hasParam("ins_lon_ref"))
            {
                gps_init_ = false;
                ROS_ERROR("Missing parameter for Inertial Sense: ~ins_lon_ref");
            }
            if (!nh_private_.hasParam("ins_alt_ref"))
            {
                gps_init_ = false;
                ROS_ERROR("Missing parameter for Inertial Sense: ~ins_alt_ref");
            }
            if (gps_init_)
            {
                init_lat_ = nh_private_.param<double>("ins_lat_ref",0);
                init_lon_ = nh_private_.param<double>("ins_lon_ref",0);
                init_alt_ = nh_private_.param<double>("ins_alt_ref", 0);
            }
            inertial_sense_sub_ = nh_.subscribe(inertial_sense_topic, 1, &estimator_base::inertialSenseCallback,
                                                this);
            update_timer_ = nh_.createTimer(ros::Duration(1.0 / update_rate_),
                                            &estimator_base::updateAltitudeAndAirspeed, this);
